...
import { faHome, faClipboardList, faTable, faCog, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';
...
const NavItem = ({ to, icon, label, currentPath }) => {
  ...
  return (
    <Link to={to} className={`flex flex-col md:flex-row items-center p-3 md:px-6 md:py-4 transition-all border-b-2 md:border-b-0 md:border-l-4 ${isActive ? 'text-primary border-primary bg-darker/50' : 'text-gray-500 border-transparent hover:text-primary hover:bg-darker/30'}`}>
      ...
      <span className="text-[10px] md:text-sm font-bold tracking-wide uppercase">{label}</span>
    </Link>
  );
};
...
function AppNavigation() {
  ...
  return (
    <div className="flex md:flex-col w-full justify-around md:justify-start md:mt-4">
      <NavItem to="/" icon={faHome} label="Beranda" currentPath={location.pathname} />
      <NavItem to="/forms" icon={faClipboardList} label="Data Form" currentPath={location.pathname} />
      <NavItem to="/responses" icon={faTable} label="Hasil Input" currentPath={location.pathname} />
      <NavItem to="/settings" icon={faCog} label="Pengaturan" currentPath={location.pathname} />
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        {/* ======================================================== */}
        {/* JALUR PUBLIK MUTLAK: BEBAS LOGIN UNTUK RESPONDEN LUAR */}
        {/* ======================================================== */}
        <Route path="/p/:id" element={<PublicForm />} />

        {/* ======================================================== */}
        {/* JALUR ADMIN ENTERPRISE: DILINDUNGI SUPABASE AUTH */}
        {/* ======================================================== */}
        <Route path="/*" element={
          !session ? <Auth /> : (
            <div className="min-h-screen bg-dark text-gray-200 font-sans flex flex-col md:flex-row">
              <nav className="fixed bottom-0 left-0 w-full md:w-64 md:h-screen bg-darker/95 backdrop-blur-xl border-t md:border-t-0 md:border-r border-gray-800 shadow-2xl z-50 flex md:flex-col justify-around md:justify-start pb-safe md:pb-0">
                <div className="hidden md:flex flex-col items-center justify-center py-8 border-b border-gray-800">
                  <div className="bg-primary/20 w-16 h-16 rounded-full flex items-center justify-center mb-3 border border-primary/30">
                ...
                <AppNavigation />
                <button onClick={() => supabase.auth.signOut()} className="hidden md:flex items-center mt-auto p-6 text-red-500 hover:text-red-400 hover:bg-darker transition-colors border-t border-gray-800">
              ...
              <main className="flex-1 p-4 md:p-8 pb-24 md:pb-8 md:ml-64 w-full max-w-[100vw]">
                <Routes>
                  <Route path="/" element={<Dashboard />} />
                  <Route path="/forms" element={<FormList />} />
                  <Route path="/form/:id" element={<SmartForm userProfile={{ id: session.user.id, kabupaten: 'Tapin' }} isAdmin={true} />} />
